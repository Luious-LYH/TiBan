...
不要把 `.env`、真实 key、服务器密码或患者身份信息提交到 git。模型准入页的一次性 key 可用于轻量自检或样例级准入检查，不会保存到数据文件；只有填写临时 base 或 key 时，页面模型名才会随本次请求覆盖后端 `.env` 默认值。
...
12. 模型准入 `/models`
    先做 Provider 轻量自检：它只发送一条安全短提示词验证通道，不读取公开样例，不保存 key/base/完整回复，也不更新模型准入状态。随后用最多 3 个公开样例和可选 Provider 做样例级准入检查；Provider 收到的是图片和问题，不包含参考标注，后端只在返回后做公开标注粗粒度对齐。只有真实调用成功才显示 `provider_called=true`；只有调用成功且至少一条盲测回答与公开标注部分对齐，最近准入状态才会标为可进入人工复核。
...
| 模型准入 | OpenAI-compatible Provider 轻量自检 + 公开样例 blind probe 准入检查 | 自检只验证通道并写摘要审计；样例准入不向 Provider 泄露参考标注；检查清单分只服务训练 Agent 接入，不是完整临床评测，不包含批量统计置信区间 |
