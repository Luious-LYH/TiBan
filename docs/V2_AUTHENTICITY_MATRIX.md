...
|---|---|---|---|---|
| 训练题库 | 公开样例优先、支持筛选/收藏/错题 | `questions.json`, `real_sample_knowledge.json`, `learner_profile.json` | 右侧 chat 可调用 Provider，失败后规则辅导 | 考试 session 计时仍可增强 |
| 右侧 Agent | `辅导/证据/对照` 三段式面板，提交前隐藏参考答案 | 当前题、atomic facts、公开图片 URL | `tutor_orchestrator.chat` 调用 Provider | Agent 对话尚未回灌画像 |
| 报告中心 | 公开样例、上传图片、结构化报告草稿、幻觉审查 | 医生输入、公开样例标注、模板 KB、上传图片 | `report_service` 可生成视觉/文本观察摘要 | 报告 judge 仍是规则评分 |
...
| Memory | 提交题目后更新准确率、错题、能力分 | `learner_profile.json` | 不调用 Provider | 报告训练、Agent 追问、准入结果未统一写入画像 |
