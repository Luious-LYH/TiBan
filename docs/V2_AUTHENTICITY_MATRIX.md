...
| 科普卡片 | 卡片模板、真实样例图片预览、动画卡片、本机图片预览、报告修改训练来源提示、医生审核闸门、审核后打印/分享解锁 | `card_template_knowledge.json` + 医师输入摘要、报告 judge 建议改写或医生修改稿摘要 + 审计日志 | 暂不调用 Provider | 从报告训练带入的摘要通过前端路由状态传递，不写入 URL；本机上传图只保留当前浏览器预览，不写入后端卡片记录；后续可接报告中心已审核结论一键带入 |
...
报告修改训练到科普卡片的入口只把 AI judge 建议改写或医生修改稿摘要作为“患者沟通卡片草稿输入”，不会自动生成已审核患者材料。科普卡片的打印/分享不是自由按钮：草稿生成后默认 `share_status=locked_pending_review`，医生完成审核清单后调用 `/patient-card/{card_id}/approve` 审核同一张草稿，后端返回 `reviewed_ready_to_share` 才会解锁，同时写入 `patient_card_approve` 审计事件。
