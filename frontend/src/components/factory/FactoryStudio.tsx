...
import { createQuestionImportBatch, deleteQuestionImportBatch, getDomains, getQuestionBanks, getQuestionImportBatch, getQuestionImportBatches, getQuestionImportTemplates, publishQuestionImportBatch, reviewQuestionImportBatch, reviewQuestionImportDraft, validateQuestionBankImport } from '../../api/client'
import type { QuestionBank, QuestionImportBatch, QuestionImportDraft, QBankValidation } from '../../api/client'
...
function GeneratePanel({ onCreated }: { onCreated: (batch: QuestionImportBatch) => void }) {
  ...
  async function chooseFile(file?: File) {
    ...
    try {
      ...
      setContent(await readTextFile(file)); setFileName(file.name); setSourceName((current) => current || file.name.replace(/\.[^.]+$/, '')); setFormat(detected); resetPreview()
    } catch (reason) { setError((reason as Error).message) }
  }

  async function validate() {
    setBusy(true); setError(null)
    try { setValidation(await validateQuestionBankImport({ format, content, source_name: sourceName || '个人导入题库' })) } catch (reason) { setError((reason as Error).message) } finally { setBusy(false) }
  }
