...
import { Download, Eye, EyeOff, LoaderCircle, Play, RefreshCw, ServerCog } from 'lucide-react'
import { useState } from 'react'

import { createEvaluationSuite, createModelEvaluation, deleteEvaluationExperiments, discoverEvaluationModels, getEvaluationExperiment, getLatestEvaluationExperiment, getLatestEvaluationSuite, type EvalSuite, type EvaluationCatalog, type EvaluationExperiment } from '../../api/client'
...
export function ModelEvaluationTab({ catalog }: { catalog: EvaluationCatalog }) {
  ...
  const discoverModels = useMutation({
    mutationFn: () => discoverEvaluationModels({ base_url: baseUrl.trim(), api_key: apiKey, api_format: 'openai' }),
    onSuccess: (result) => {
      setCandidateText(result.models.map((model) => model.id).join('\n'))
      setDiscoveryMessage(`已填入 ${result.models.length} 个模型`)
    },
  })
  ...
  const canDiscover = connectionMode === 'custom' && customReady && !discoverModels.isPending
  ...
  return <section className="evaluation-lab" data-testid="model-evaluation-tab">
    <div className="evaluation-panel evaluation-setup evaluation-model-setup">
      ...
      <div className="evaluation-field evaluation-candidates-field"><div className="evaluation-candidates-heading"><span>04 · 候选模型</span><button type="button" className="evaluation-discovery-button" disabled={!canDiscover} onClick={() => { setDiscoveryMessage(null); discoverModels.mutate() }}><Download size={14} className={discoverModels.isPending ? 's1-spin' : ''} />{discoverModels.isPending ? '正在获取…' : '从上游获取模型'}</button></div><textarea aria-label="候选模型" value={candidateText} onChange={(event) => { setCandidateText(event.target.value); setDiscoveryMessage(null) }} placeholder="每行填写一个模型名称" /><div className="evaluation-candidates-footer"><small>{discoveryMessage ?? '每行填写一个模型；结果显示短名称，评测请求仍使用完整模型 ID。'}</small><button className="evaluation-primary" type="button" disabled={!canRun} onClick={() => createExperiment.mutate()}><Play size={15} />开始模型评测</button></div></div>
      {(latestSuite.isError || latestExperiment.isError || createSuite.error || resample.error || createExperiment.error || discoverModels.error) && <div className="evaluation-action-row evaluation-error-row"><div>{latestSuite.isError && <p className="evaluation-error" role="alert">{errorMessage(latestSuite.error)} 可直接创建新的评测集。</p>}{latestExperiment.isError && <p className="evaluation-error" role="alert">已保存的模型评测结果暂时无法读取。</p>}{createSuite.error && <p className="evaluation-error" role="alert">{errorMessage(createSuite.error)}</p>}{resample.error && <p className="evaluation-error" role="alert">{errorMessage(resample.error)}</p>}{createExperiment.error && <p className="evaluation-error" role="alert">{errorMessage(createExperiment.error)}</p>}{discoverModels.error && <p className="evaluation-error" role="alert">{errorMessage(discoverModels.error)} 可继续手动填写模型。</p>}</div></div>}
    </div>
