...
import { createCoachConversation, getCoachConversation, getKnowledgeSources, listCoachConversations, streamCoachMessage, type CoachMessage, type TutorStreamEvent } from '../../api/client'
import { EmptyState, ErrorState, LoadingState } from '../../components/shared/AsyncState'

type CoachSource = { document_name?: string; section?: string; page?: string; snippet?: string }
type LiveMessage = { id: string; role: 'user' | 'assistant'; content: string; activity?: Array<{ label?: string; status?: string }>; sources?: CoachSource[] }
const starters = ['根据我最近的错题，告诉我接下来应该复习什么', '总结我最近容易混淆的知识点', '帮我制定今天 30 分钟的刷题计划']

export function CoachPage() {
  const query = useQueryClient()
  const conversations = useQuery({ queryKey: ['coach-conversations'], queryFn: () => listCoachConversations() })
  const [selectedId, setSelectedId] = useState<string | null>(null)
  const effectiveId = selectedId ?? conversations.data?.[0]?.id ?? null
  const detail = useQuery({ queryKey: ['coach-conversation', effectiveId], queryFn: () => getCoachConversation(effectiveId ?? ''), enabled: Boolean(effectiveId) })
  ...
  const create = useMutation({ mutationFn: createCoachConversation, onSuccess: (item) => { setSelectedId(item.id); void query.invalidateQueries({ queryKey: ['coach-conversations'] }) } })

  const savedMessages = useMemo(() => (detail.data?.messages ?? []) as CoachMessage[], [detail.data?.messages])
  ...
  async function send(value = draft) {
    ...
    try {
      await streamCoachMessage(conversationId, text, (event) => handleEvent(event, assistantId), next.signal)
      await query.invalidateQueries({ queryKey: ['coach-conversations'] })
      await query.invalidateQueries({ queryKey: ['coach-conversation', conversationId] })
    } catch (error) {
  ...
  function handleEvent(event: TutorStreamEvent, assistantId: string) {
    setLive((current) => current.map((item) => {
      ...
      if (event.event === 'source') return { ...item, sources: [...(item.sources ?? []), event.data as CoachSource] }
  ...
  return <div className="coach-page" data-testid="coach-page">
    <aside className="coach-history"><header><div><span>Agent</span><h1>带教 Agent</h1></div><button type="button" onClick={() => void startConversation()} disabled={create.isPending} aria-label="新建带教对话"><Plus size={17} /></button></header><p>跨题库读取你的作答、复习安排和学习记忆。</p><div className="coach-history-label">最近对话</div><nav>{(conversations.data ?? []).length === 0 ? <p className="coach-history-empty">还没有带教对话</p> : conversations.data?.map((item) => <button type="button" key={item.id} className={item.id === effectiveId ? 'is-selected' : ''} onClick={() => { loadedConversation.current = null; setSelectedId(item.id); setLive([]) }}><span>{item.title}</span><small>{formatTime(item.updated_at)}</small></button>)}</nav></aside>
    <main className="coach-conversation"><header><div><span className="coach-avatar"><Sparkles size={17} /></span><div><strong>带教 Agent</strong><small>基于真实学习记录与已启用资料</small></div></div>{running && <span className="coach-running"><LoaderCircle className="s1-spin" size={14} />正在整理</span>}</header><div className="coach-transcript" ref={transcript} onScroll={(event) => { const node = event.currentTarget; setFollow(node.scrollHeight - node.scrollTop - node.clientHeight < 36) }}>
      {!effectiveId && <CoachEmpty onSend={(text) => void send(text)} />}
      ...
      {messages.map((item) => <CoachTurn key={item.id} item={item} />)}
    </div>{!follow && <button className="coach-jump" type="button" onClick={() => { setFollow(true); transcript.current?.scrollTo({ top: transcript.current.scrollHeight, behavior: 'smooth' }) }}>回到底部</button>}
    ...
    <aside className="coach-knowledge"><header><BookOpenText size={17} /><div><strong>已启用资料</strong><small>带教 Agent 仅在需要时检索</small></div></header><div>{knowledge.isPending ? <LoadingState label="正在读取资料…" /> : enabledSources.length === 0 ? <EmptyState title="没有启用资料" detail="到知识库添加或启用一份学习资料。" /> : enabledSources.map((source) => <article key={source.id}><FileText size={16} /><div><strong>{source.title}</strong><small>{source.scope === 'user' ? '我的资料' : source.scope === 'qbank_explanations' ? '题库解析' : '系统资料'} · {source.chunk_count} 个片段</small></div></article>)}</div><Link to="/knowledge">管理知识库 <ChevronRight size={14} /></Link></aside>
  </div>
}

function CoachEmpty({ onSend }: { onSend: (text: string) => void }) { return <section className="coach-empty"><span className="coach-empty-mark"><MessageSquarePlus size={22} /></span><h2>今天想从哪里开始？</h2><p>我会在需要时读取真实的学习记录、复习队列和已启用资料。</p><div>{starters.map((item) => <button type="button" key={item} onClick={() => onSend(item)}>{item}<ChevronRight size={15} /></button>)}</div></section> }
function CoachTurn({ item }: { item: LiveMessage }) { const evidence = dedupe(item.sources ?? []); const completed = (item.activity ?? []).filter((entry) => entry.status === 'completed'); return <article className={`coach-turn is-${item.role}`}><span>{item.role === 'user' ? '你' : '带教 Agent'}</span>{item.content && <p>{item.content}</p>}{completed.length > 0 && <details><summary>{completed.map((entry) => entry.label).filter(Boolean).join(' · ')}</summary></details>}{evidence.length > 0 && <div className="coach-evidence">{evidence.map((source, index) => <article key={`${source.document_name}-${source.section}-${index}`}><strong>{source.document_name ?? '学习资料'}</strong><small>{source.section ?? source.page}</small><p>{source.snippet?.slice(0, 160)}</p></article>)}</div>}</article> }
function toLiveMessage(item: CoachMessage): LiveMessage { return { id: item.id, role: item.role, content: item.content, activity: (item.activity ?? []) as LiveMessage['activity'], sources: (item.sources ?? []) as LiveMessage['sources'] } }
function dedupe(sources: CoachSource[]) { const seen = new Set<string>(); return sources.filter((source) => { const key = `${source.document_name ?? ''}:${source.section ?? ''}`; if (seen.has(key)) return false; seen.add(key); return true }) }
