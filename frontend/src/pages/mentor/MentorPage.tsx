...
type MentorSource = { document_name?: string; section?: string; page?: string; snippet?: string; image_urls?: string[]; show_in_chat?: boolean }
...
function MentorTurn({ item, isStreaming = false }: { item: LiveMessage; isStreaming?: boolean }) { const evidence = dedupe(item.sources ?? []); const replyImages = collectReplyImages(evidence); const completed = (item.activity ?? []).filter((entry) => entry.status === 'completed'); return <article className={`mentor-turn is-${item.role}`}><span>{item.role === 'user' ? '你' : '带教 Agent'}</span>{item.imageUrl && <img className="mentor-turn-image" src={item.imageUrl} alt="你附加的图片" />}{item.imageAttached && !item.imageUrl && <small className="mentor-image-status">已附图片</small>}{item.content && <p>{item.content}</p>}{replyImages.length > 0 && <section className="mentor-reply-images" aria-label="相关资料图片"><strong>相关资料图片</strong><div>{replyImages.map((image) => <figure key={image.url}><a href={resolveApiUrl(image.url)} target="_blank" rel="noreferrer" aria-label={`查看${image.documentName}第 ${image.page} 页图片`}><img src={resolveApiUrl(image.url)} alt={`${image.documentName} · 第 ${image.page} 页`} loading="lazy" /></a><figcaption><span>{image.documentName} · 第 {image.page} 页</span>{image.caption && <small>{image.caption}</small>}</figcaption></figure>)}</div></section>}{item.role === 'assistant' && isStreaming && <div className="mentor-answer-status" role="status" aria-live="polite"><LoaderCircle className="s1-spin" size={14} /><span>正在生成回答…</span><i className="tutor-status-dots" aria-hidden="true">•••</i></div>}{completed.length > 0 && <details><summary>{completed.map((entry) => entry.label).filter(Boolean).join(' · ')}</summary></details>}{evidence.length > 0 && <details className="mentor-evidence"><summary>参考资料 {evidence.length} 条</summary><div>{evidence.map((source, index) => <article key={`${source.document_name}-${source.section}-${index}`}><strong>{source.document_name ?? '学习资料'}</strong><small>{source.section ?? source.page}</small><p>{source.snippet?.slice(0, 160)}</p>{!source.show_in_chat && source.image_urls && source.image_urls.length > 0 && <div className="mentor-evidence-images">{source.image_urls.slice(0, 3).map((url, imageIndex) => <a key={url} href={resolveApiUrl(url)} target="_blank" rel="noreferrer" aria-label={`查看资料图片 ${imageIndex + 1}`}><img src={resolveApiUrl(url)} alt={`${source.document_name ?? '资料图片'} · 第 ${source.page ?? '?'} 页`} loading="lazy" /></a>)}</div>}</article>)}</div></details>}</article> }
...
function dedupe(sources: MentorSource[]) { const seen = new Set<string>(); return sources.filter((source) => { const key = `${source.document_name ?? ''}:${source.section ?? ''}:${(source.image_urls ?? []).join('|')}`; if (seen.has(key)) return false; seen.add(key); return true }) }
function collectReplyImages(sources: MentorSource[]) { const seen = new Set<string>(); return sources.flatMap((source) => source.show_in_chat ? (source.image_urls ?? []).map((url) => ({ url, documentName: source.document_name ?? '学习资料', page: source.page ?? '?', caption: source.snippet?.slice(0, 84) ?? '' })) : []).filter((image) => { if (seen.has(image.url)) return false; seen.add(image.url); return true }).slice(0, 3) }
