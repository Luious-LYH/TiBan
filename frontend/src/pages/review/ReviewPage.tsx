import { Bookmark, CalendarClock, ChevronRight, CircleAlert, RotateCcw } from 'lucide-react'
...
export function ReviewPage() {
  ...
  return <div className="review-page" data-testid="review-page">
    ...
    {summary.isPending ? <ReviewSummarySkeleton /> : summary.isError ? <ReviewSummaryError message={summary.error.message} onRetry={() => void summary.refetch()} /> : <section className="review-summary" aria-label="复习摘要"><Summary icon={<CalendarClock />} label="待复习" value={summary.data.due_count} /><Summary icon={<CircleAlert />} label="错题" value={summary.data.incorrect_count} /><Summary icon={<Bookmark />} label="已标记" value={summary.data.marked_count} /></section>}
    ...
    <section className="review-workspace" aria-busy={items.isPending}>
      <div className="review-list-pane">{items.isPending ? <ReviewListSkeleton /> : items.isError ? <ErrorState message={items.error.message} onRetry={() => void items.refetch()} /> : !items.data?.items.length ? <EmptyState title="当前没有可复习的题目" detail="完成练习、标记题目或等待复习到期后，它们会出现在这里。" /> : <ol className="review-list">{items.data.items.map((item) => <ReviewListItem key={item.question_id} item={item} selected={item.question_id === selectedItemId} onSelect={() => setSelectedId(item.question_id)} />)}</ol>}</div>
...
function ReviewListItem({ item, selected, onSelect }: { item: ReviewItem; selected: boolean; onSelect: () => void }) { return <li><button type="button" className={selected ? 'is-selected' : ''} onClick={onSelect}><span className={item.incorrect ? 'is-wrong' : ''}>{item.incorrect ? <CircleAlert size={14} /> : <RotateCcw size={14} />}</span><div><strong>{item.question_summary}</strong><small>{item.bank_name} · {item.wrong_count ? `错 ${item.wrong_count} 次` : '已标记'}</small></div>{item.due_at && <time>{new Date(item.due_at).toLocaleDateString('zh-CN', { month: 'numeric', day: 'numeric' })}</time>}</button></li> }
