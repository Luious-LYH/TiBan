...
function SourceRow({ source, selected, onSelect }: { source: KnowledgeSource; selected: boolean; onSelect: () => void }) { return <li><button type="button" className={selected ? 'is-selected' : ''} onClick={onSelect}><span><FileText size={17} /></span><div><strong>{source.title}</strong><small>{fileLabel(source.media_type)} · {isIndexing(source) ? `${source.index_stage ?? '正在索引'} ${source.index_progress}%` : `${source.chunk_count} 个片段`}</small></div><i className={stateClass(source.status)}>{stateLabel(source)}</i></button></li> }
