...
export function KnowledgePage() {
  ...
  return <div className="knowledge-page" data-testid="knowledge-page">
    ...
    <section className="knowledge-workspace">
      <div className="knowledge-source-list">{sources.isPending ? <KnowledgeListSkeleton /> : sources.isError ? <ErrorState message={sources.error.message} onRetry={() => void sources.refetch()} /> : visible.length === 0 ? <EmptyState title={tab === 'user' ? '还没有个人资料' : '系统资料准备中'} detail={tab === 'user' ? '上传一份学习资料后，它会在索引完成后供 Agent 检索。' : '系统资料只保留经过许可、可追溯的少量来源。'} /> : <ol>{visible.map((source) => <SourceRow key={source.id} source={source} selected={source.id === effectiveSelected} onSelect={() => setSelectedId(source.id)} onDelete={() => requestDelete(source)} deleting={remove.isPending} />)}</ol>}</div>
      <article className="knowledge-detail">{sources.isPending ? <KnowledgeDetailSkeleton /> : sources.isError ? <EmptyState title="资料列表暂不可用" detail="请重试读取知识库目录后再查看资料详情。" /> : !effectiveSelected ? <EmptyState title="选择一份资料查看详情" detail="这里只展示已真实解析和索引的来源。" /> : detail.isPending ? <LoadingState label="正在读取资料详情…" /> : detail.isError ? <ErrorState message={detail.error.message} onRetry={() => void detail.refetch()} /> : detail.data && <><header><div><span>{scopeLabel(detail.data.scope)}</span><h2>{detail.data.title}</h2><p>{fileLabel(detail.data.media_type)} · {formatSize(detail.data.size_bytes)} · {detail.data.chunk_count} 个片段</p></div><MoreHorizontal size={19} /></header><div className="knowledge-state"><span className={stateClass(detail.data.status)}>{stateLabel(detail.data)}</span>{detail.data.attribution && <small>{detail.data.attribution}</small>}</div>{isIndexing(detail.data) && <p className="knowledge-index-progress" role="status">{detail.data.index_stage ?? '正在准备索引'} · {detail.data.index_progress}%</p>}{detail.data.index_error && <p className="knowledge-error" role="alert">索引失败：{detail.data.index_error}</p>}<div className="knowledge-actions"><button type="button" onClick={() => toggle.mutate({ id: detail.data.id, enabled: !detail.data.enabled })} disabled={toggle.isPending || isIndexing(detail.data)}>{detail.data.enabled ? <ToggleRight size={16} /> : <ToggleLeft size={16} />}{detail.data.enabled ? '停用' : '启用'}</button><button type="button" onClick={() => reindex.mutate(detail.data.id)} disabled={reindex.isPending || isIndexing(detail.data)}>{reindex.isPending ? <LoaderCircle className="s1-spin" size={16} /> : <RefreshCw size={16} />}重新索引</button>{detail.data.scope === 'user' && <button type="button" className="is-danger" onClick={() => { if (window.confirm(`删除「${detail.data.title}」及其索引？`)) remove.mutate(detail.data.id) }} disabled={remove.isPending}><Trash2 size={16} />删除</button>}</div><section className="knowledge-preview"><h3>解析预览</h3>{detail.data.preview?.length ? detail.data.preview.map((item, index) => <article key={`${String(item.section)}-${index}`}><small>{previewLabel(item.section, item.page)}</small><p>{String(item.text)}</p></article>) : <p>{isIndexing(detail.data) ? '资料正在后台解析，完成后将在这里显示真实片段。' : '当前资料没有可显示的解析片段。'}</p>}</section></>}</article>
    </section>
  </div>
}
...
function SourceRow({ source, selected, onSelect, onDelete, deleting }: { source: KnowledgeSource; selected: boolean; onSelect: () => void; onDelete: () => void; deleting: boolean }) {
  return <li className="knowledge-source-row"><button type="button" className={selected ? 'is-selected' : ''} onClick={onSelect}><span><FileText size={17} /></span><div><strong>{source.title}</strong><small>{fileLabel(source.media_type)} · {isIndexing(source) ? `${source.index_stage ?? '正在索引'} ${source.index_progress}%` : `${source.chunk_count} 个片段`}</small></div><i className={stateClass(source.status)}>{stateLabel(source)}</i></button>{source.scope === 'user' && <button type="button" className="knowledge-source-delete" aria-label={`删除${source.title}`} onClick={(event) => { event.stopPropagation(); onDelete() }} disabled={deleting}><Trash2 size={15} aria-hidden="true" /></button>}</li>
}
