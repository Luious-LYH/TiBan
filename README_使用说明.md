...
9. 科普卡片页已加入医生审核闸门：草稿默认锁定打印/分享，医生勾选审核项并提交后才会解锁，并写入审计日志。
...
- 报告中心首屏会展示数据来源、Provider 和模板状态；公开 VQA 标注默认收进来源台账，不伪装成医生报告结论。
- 模型准入已从单样例探测升级为样例级准入检查清单：每个公开样例都会返回 evidence、Provider 调用状态和错误原因；分数只代表训练接入检查，不代表临床评测。
- 科普卡片已从单纯预览升级为“草稿生成 -> 医生审核 -> 分享/打印解锁 -> 审计记录”的受控流程。
