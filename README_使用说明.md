...
2. 训练中心右侧 Agent 分为“辅导 / 证据 / 对照”，提交前隐藏参考答案；追问会记录训练标签和模式，不保存自由追问原文。
3. 报告中心会区分医生输入、公开样例标注、模板知识库、Provider 输出。
4. 报告修改训练提交后会回灌林知远医师画像，更新训练记录、能力分和弱项标签。
5. 考试模式有 12 分钟倒计时，提交后解锁公开标注/AI 对照。
6. 上传图片会保存到 `backend/runtime/uploads`，该目录已加入 `.gitignore`。
7. 模型准入页可使用后端 `.env` 或页面临时 key 进行一次真实探测，但不会保存密钥；最近准入摘要会显示在模型页和训练驾驶舱。
8. 真实性说明见 `docs/V2_AUTHENTICITY_MATRIX.md`。
