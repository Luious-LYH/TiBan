...
source_url: https://www.niddk.nih.gov/health-information/diagnostic-tests/colonoscopy
source_title: NIDDK Colonoscopy
...
本条用于消化内镜和消化系统知识学习，主题为“结肠镜检查概念”。它将公开健康教育资料中的概念整理为可检索的中文学习要点；原始来源见文末链接。它不复制上游页面全文，也不包含图像、商标或第三方受限内容。

## 证据要点

- 结肠镜用于观察直肠和结肠内部。
- 镜下描述应先给出观察范围、部位和可见发现。
- 检查相关教学资料不应被用于指导个人准备或处置。

## 题目解析如何使用

面对“colonoscopy”相关题干，先确认题目问的是概念、症状线索、检查证据还是镜下观察。再把题干中直接给出的部位、时间、检查结果或组织学信息与本条的证据层级对应。没有写在题干或资料中的事实，应保留为未知，而不是由模型补全。
...
> 来源：[NIDDK Colonoscopy](https://www.niddk.nih.gov/health-information/diagnostic-tests/colonoscopy)。NIDDK 版权页说明其网站上大部分信息可在保留署名且排除例外内容的前提下复用。本条为 EndoTutor 项目中文教学摘要，非官方翻译。仅供教学训练或医生审核前辅助，不作为独立诊断依据。
