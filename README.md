...
7. 模型准入：先做 Provider 轻量自检确认通道可用，再使用最多 3 个公开样例做样例级真实/规则准入检查，查看逐样例 Provider evidence、延迟、风险项和平台最近准入摘要。
...
| 模型准入 | provider self-test / provider probe / rule draft | 公开样例 + 可选 Provider | 轻量自检只验证通道并写摘要审计，不更新准入状态；逐样例准入返回 evidence，真实调用成功才标记 `provider_called=true`；最近准入摘要会写入平台状态，不保存 key/base |
